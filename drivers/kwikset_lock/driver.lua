...
-- ZCL DoorLock OperationEventSource (the first payload byte) -> who acted. This
-- is a distinct field from the event code; reading the code for the source is
-- what mislabeled a keypad unlock.
...
-- A Zigbee 3.0 device pushes unsolicited operation events and attribute reports
-- only to destinations in its own binding table, and nothing populates that
-- table on join. Without a binding the lock answers polls but never announces a
-- keypad or thumbturn action on its own. lib/zbind establishes the binding to
-- the controller's coordinator through zserver's public API; the binding lives
-- in the lock and persists across reloads, so this runs once per driver load
-- and the status poll stays as the fallback.
