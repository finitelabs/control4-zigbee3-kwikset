...
function ZBind:drain()
  ...
  for _ = 1, DRAIN_MAX do
    ...
    if err == "closed" then
      -- Broker dropped the connection mid-flow. Parse before settling: the
      -- completing frame can arrive in the same read as the close. Settle with
      -- finish(), not close() - close() alone leaves state non-idle with the
      -- callback never called, so every later ensureBinds short-circuits on the
      -- idle guard.
      pcall(function()
        self:parse()
      end)
      if self.state ~= "idle" then
        self:finish(false, "broker closed")
      end
      return
    end
  end
