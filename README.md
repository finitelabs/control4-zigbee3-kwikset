...
- A controller running a **Zigbee 3.0** mesh with an open network
- A Kwikset SmartCode deadbolt or lever with a **Zigbee 3.0** radio (see
  [Compatibility](#compatibility))

> **Requires a Zigbee 3.0 mesh.** The lock is a Zigbee 3.0 device and joins the
> controller's Zigbee mesh directly. A Control4 controller runs one **Mesh
> Type** at a time, either **Zigbee 3.0** or **Zigbee Pro (Legacy)**, and this
> lock requires **Zigbee 3.0** - it will not join a Zigbee Pro (Legacy) mesh. At
> this time only CORE-series controllers can host a Zigbee 3.0 mesh, and the
> Mesh Type is chosen when the controller's Zigbee network is created. See
> Control4's
> [Using Zigbee 3.0 and Zigbee Pro together](https://help.snapone.com/lux-qs/Content/System%20Design%20Topics/Zigbee%203%20and%20Legacy.htm)
> to check or set your controller's mesh type.
...
module. Verified models:

| Series                   | Type           | Model number | Reported model ID          |
| ------------------------ | -------------- | ------------ | -------------------------- |
| SmartCode 5              | Deadbolt       | 99100-045    | SMARTCODE_DEADBOLT_5       |
| SmartCode 5              | Deadbolt       | 99100-006    | SMARTCODE_DEADBOLT_5_L     |
| SmartCode 5              | Lever          | 99120-021    | SMARTCODE_LEVER_5          |
| SmartCode 10             | Deadbolt       | 99140-002    | SMARTCODE_DEADBOLT_10_L    |
| SmartCode 10             | Deadbolt       | 99140-031    | SMARTCODE_DEADBOLT_10_W3   |
| SmartCode 10             | Deadbolt       | 99140-031    | SMARTCODE_DEADBOLT_10_W3_L |
| SmartCode 10 Touchscreen | Deadbolt       | 99140-031    | SMARTCODE_DEADBOLT_10T_W3  |
| SmartCode Convert        | Conversion kit | 99140-139    | SMARTCODE_CONVERT_GEN1_W3  |
| SmartCode Convert        | Conversion kit | 66492-001    | SMARTCODE_CONVERT_GEN1     |
| SmartCode Convert        | Conversion kit | 66492-001    | Smartcode                  |
...
1. Confirm the controller is running a **Zigbee 3.0** mesh with an open network.
...
| Command            | Description                                              |
| ------------------ | -------------------------------------------------------- |
| Sync Users         | Re-sends every stored user code and schedule to the lock |
| Get Battery Status | Requests the lock's current battery level                |
...
- Made real-time lock events more reliable: after binding the lock for live
  updates, the driver confirms the binding landed in the lock's own table and
  re-establishes it if missing, instead of silently falling back to the poll
